...
**FIRST**, verify claude-control is properly installed:

```bash
# Run quick validation
~/.claude-control/tests/validate-structure.sh --quick
```
...
- Suggest running: `cd ~/.claude-control && git pull && ./install.sh`
...
_Skip if CLAUDE.md exists and has Project Overview - show current and ask if they want to update._
...
_Auto-detect from package.json, pyproject.toml, pubspec.yaml, or android/ directory if present._
...
_Auto-detect from dependencies if possible._
...
_Check for anthropic/openai in dependencies._
...
_Auto-detect from dependencies._
...
_Skip if supabase/ directory exists._
...
_Skip if vercel.json or render.yaml exists._
...
~/.claude-control/tests/validate-structure.sh --full
...
- **Missing skills**: Reinstall with `./install.sh`
