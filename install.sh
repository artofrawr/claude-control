...
# Claude Control Bootstrap Installer

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CLAUDE_DIR="$HOME/.claude"
BOOTSTRAP_DIR="$HOME/.claude-control"

echo "Running Claude Control Bootstrap..."
echo ""
...
# Create symlink for ~/.claude-control (for validation scripts)
if [ "$SCRIPT_DIR" != "$BOOTSTRAP_DIR" ]; then
    ...
    if [ ! -d "$BOOTSTRAP_DIR" ]; then
        ...
        echo "✓ Created symlink: ~/.claude-control -> $SCRIPT_DIR"
    fi
fi
...
if [ -d "$CLAUDE_DIR/plugins/marketplaces/claude-plugins-official/plugins/ralph-loop" ]; then
    ...
    echo "  Update marketplace: /plugin update"
...
echo "  /initialize-project   - Project setup"
echo "  /initialize-metarepo  - Meta repository setup"
...
echo ""
echo "Validation:"
echo "  ~/.claude-control/tests/validate-structure.sh --full"
echo ""